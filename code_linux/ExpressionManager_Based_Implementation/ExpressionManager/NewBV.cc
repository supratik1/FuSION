...
bvatom BVManager::reduction_and(bvatom bv_inst1)
{
  ...
  if(flag==1)
	...
	{ /* last_offset == avail_block_size - 1 */
...
bvatom BVManager::select_with_int(bvatom bv_inst1, unsigned long hi, unsigned long lo)
{
  if (hi < lo) {
    ...
    assert(0);
#else
    cout << "Swapping! hi (" << hi << ") of select_with_bv < lo (" << lo << ")" << endl;
...
/*The following functions from addInverse to bvsub are added by Ajith John */

// Given bv_inst of width w bits, this function should 
// return additive inverse of bv_inst modulo 2^{w}. 
bvatom BVManager::addInverse(bvatom bv_inst)
{
   // just call arith_neg
   return arith_neg(bv_inst);
   
} 


// Given bv_inst of width w bits, this function should 
// return multiplicative inverse of bv_inst modulo 2^{w}. 
bvatom BVManager::multInverse(bvatom bv_inst)
{
   // if bv_inst is even, then, there is no mutiplicative inverse; 
   // return zero with error message 
   if (checkBVForZero(select_with_int(bv_inst, 0, 0))) 
   {	
	cerr<<"Error in function BVManager::multInverse; called with even argument\n";
	bvatom inverse = getZeroedBVatom(bv_inst.bv_width);
   	return(inverse);
   }
   else
   {	
	
	// TODO: Memoization of results, as done in STP code. 
	// Correct this first: The present hash table implementation supports
	// only hash tables with t_DAGNode* and t_Expression*. Correct this, so
	// that we can put bvatom* in the hash tables. 
	
	// using the extended Euclidian algorithm (obtained from STP's code)
	bvatom zero_width_plus_one = getZeroedBVatom(bv_inst.bv_width+1); //zero of width bv_inst.bv_width+1
	bvatom c = zero_width_plus_one; //c = 0 (width = bv_inst.bv_width+1)
	bvatom max = zero_width_plus_one; //max = 0 (width = bv_inst.bv_width+1)
	bvatom inverse = getZeroedBVatom(bv_inst.bv_width); //inverse = 0 (width = bv_inst.bv_width)
	string one_string = "1";
	bvatom one_width_plus_one = BVExtend(getBVatom(1, one_string), bv_inst.bv_width+1, 0); //one of width bv_inst.bv_width+1
	bvatom one_width = BVExtend(getBVatom(1, one_string), bv_inst.bv_width, 0); //one of width bv_inst.bv_width
	bvatom zero_bit = getZeroedBVatom(1); //zero of width 1
	
	c = concat(zero_bit, bv_inst); //c = zero of width 1 @ bv_inst
	max = bvadd(concat(zero_bit, addInverse(one_width)), one_width_plus_one); //max = "1 followed by all 0's", where the 
											      // total width is bv_inst.bv_width+1
	bvatom x1 = zero_width_plus_one; //x1 = 0 (width = bv_inst.bv_width+1)
	bvatom x2 = one_width_plus_one; //x2 = 1 (width = bv_inst.bv_width+1)
	bvatom quotient = zero_width_plus_one; //quotient = 0 (width = bv_inst.bv_width+1)
	bvatom remainder = zero_width_plus_one; //remainder = 0 (width = bv_inst.bv_width+1)
	bvatom x = zero_width_plus_one; //x = 0 (width = bv_inst.bv_width+1)
		
	while(checkBVGreaterThan(max, zero_width_plus_one))//as long as max > 0
	{
		quotient = bvdiv(c, max); // quotient = c / max		
		remainder = bvmod(c, max); // remainder = c % max
		x = bvsub(x2, bvmul(quotient, x1)); // x = x2 - quotient * x1
		c = max;
		max = remainder;
		x2 = x1;
		x1 = x;			
	...
	inverse = select_with_int(x2, bv_inst.bv_width-1, 0); // inverse = x2[bv_inst.bv_width-1 : 0]
	return(inverse);
   }	
}


// Given a value, this function finds the kappa value and the odd part.
// Returns false in case of errors; true otherwise. 
// Note that if c = 2^{k}.e, (where e is odd) is the value, then kappa = k and OddPart = e. 
bool BVManager::findKappaAndOddPart(bvatom bv_inst, unsigned int &Kappa, bvatom &OddPart)
{
	//if bv_inst is zero, then Kappa is not defined
	if (checkBVForZero(bv_inst)) 
   	{	
		cerr<<"Error in function BVManager::findKappaAndOddPart; argument is zero\n";
		return false;
  	 }	
	else
	{
		unsigned int location = 0;
		while(checkBVForZero(select_with_int(bv_inst, location, location)))
		{
			location++;
		}
		...
		/* Last parameter = 1 indicates width of bitvector to be preserved */
		OddPart = rightshiftBVatom(bv_inst, Kappa, (unsigned int) 1);
		return true;
	}
}
...
// Variant of findKappaAndOddPart which computes only the kappa value
bool BVManager::findKappa(bvatom bv_inst, unsigned int &Kappa)
{
	//if bv_inst is zero, then Kappa is not defined
	if (checkBVForZero(bv_inst)) 
   	{	
		cerr<<"Error in function BVManager::findKappa; argument is zero\n";
		return false;
  	 }	
	else
	{
...
// Given bv_inst of width w bits, this function should left shift bv_inst
// shift times, and return result of width w bits
bvatom BVManager::bvlshift(bvatom bv_inst, unsigned int shift)
{
	/* Last parameter = 1 indicates width of bitvector to be preserved */
	unsigned int w = bv_inst.bv_width;
	bvatom result = leftshiftBVatom(bv_inst, shift, (unsigned int) 1);
	return select_with_int(result, w-1, 0); // this should not be required
	// logically, but without this, the answer returned is not correct.  
}


// Given bv_inst of width w bits, this function should right shift bv_inst
// shift times, and return result of width w bits
bvatom BVManager::bvrshift(bvatom bv_inst, unsigned int shift)
{
	/* Last parameter = 1 indicates width of bitvector to be preserved */
	unsigned int w = bv_inst.bv_width;
...
// Given bv_inst1 and bv_inst2 of the same width (say w bits), 
// this function should perform unsigned addition bv_inst1 + bv_inst2
// and return the least significant w bits of the result.
bvatom BVManager::bvadd(bvatom bv_inst1, bvatom bv_inst2)
{
	...
	unsigned int w = result.bv_width;

	//result should be of width w1	
	if(w == w1)
		{
		return result;
		}
		...
		{
		return select_with_int(result, w1-1, 0);
		}
	else //i.e. w < w1
		{
		/* zero-extension to make the width w1 */
		return BVExtend(result, w1, 0);
		}
}


// Given bv_inst1 and bv_inst2 of the same width (say w bits), 
// this function should perform unsigned multiplication bv_inst1 * bv_inst2
// and return the least significant w bits of the result.
bvatom BVManager::bvmul(bvatom bv_inst1, bvatom bv_inst2)
{
	unsigned int w1 = bv_inst1.bv_width;
  	unsigned int w2 = bv_inst2.bv_width;	
	...
	bvatom result = arith_mult(bv_inst1, bv_inst2);
...
// Given bv_inst1 and bv_inst2 of the same width (say w bits), 
// this function should perform unsigned division bv_inst1 / bv_inst2
// and return the result as w bits. 
bvatom BVManager::bvdiv(bvatom bv_inst1, bvatom bv_inst2)
{
	unsigned int w1 = bv_inst1.bv_width;
  	unsigned int w2 = bv_inst2.bv_width;	
	...
	bvatom result = arith_div(bv_inst1, bv_inst2);
...
// Given bv_inst1 and bv_inst2 of the same width (say w bits), 
// this function should return the remainder of unsigned division bv_inst1 / bv_inst2
// as w bits. 
bvatom BVManager::bvmod(bvatom bv_inst1, bvatom bv_inst2)
{
	unsigned int w1 = bv_inst1.bv_width;
  	unsigned int w2 = bv_inst2.bv_width;	
	...
	bvatom result = arith_mod(bv_inst1, bv_inst2);
...
// Given bv_inst1 and bv_inst2 of the same width (say w bits), 
// this function should perform unsigned subtraction bv_inst1 - bv_inst2
// i.e. bv_inst1 + addInverse(bv_inst2) and return the 
// least significant w bits of the result. 
bvatom BVManager::bvsub(bvatom bv_inst1, bvatom bv_inst2)
{
	return bvadd(bv_inst1, addInverse(bv_inst2));
}
