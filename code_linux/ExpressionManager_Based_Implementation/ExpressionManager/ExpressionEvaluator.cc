...
bvatom t_ExpressionEvaluator::getZerosOrOnes(int number,char c)
{
    BVManager *bvm = BVManager::getInstance();
...
t_ExpressionValue* t_ExpressionEvaluator::performEvaluationOnGivenOperands(string &operatorSymbol, vector<t_ExpressionValue *> &operands)
{
    ...
    else
    {
        ...
        if (operatorSymbol == "update")
        {
            ...
            cout<<"operand value is "<<operand3->getActualValueAsString()<<endl;
        ...
        if (evaluationCompleted == true)
        {
            ...
            {
                //delete operands[2];
                //XXX: deleting the expression without removing from the hash table
            }
            return result;
        }
        ...
        else if (operatorSymbol == "rightToLeftStream")
        {
            ...
            /**
             * If only one operand is there, return the same (This is wrong and was used to patch the results with LIRA)
             */
            if (operands.size() == 1)
            {
                cout<<"ERROR!! in righttoleftstream operand size at" __FILE__" in "<<__LINE__<<endl;
//                result->m_typeInfo.m_width = operands[0]->m_typeInfo.m_width;
//                result->m_typeInfo.m_type = operands[0]->m_typeInfo.m_type;
...
//                evaluationCompleted = true;
        ...
        if (operands.size() == 1 && evaluationCompleted == false)
        {
            ...
            {
                result->m_typeInfo.m_width = op1->getWidth();//1;
                bvm->bv_copy( msb/*ZValue*/, result->m_actualExpressionValueStored.exprValue2BitVector.m_msb);
                bvm->bv_copy( lsb/*ZValue*/, result->m_actualExpressionValueStored.exprValue2BitVector.m_lsb);
                evaluationCompleted = true;
                convertResultToMemory=false;
            }
            ...
            {
                result->m_typeInfo.m_width = op1->getWidth();//1;
                bvm->bv_copy( msb/*XValue*/, result->m_actualExpressionValueStored.exprValue2BitVector.m_msb);
                bvm->bv_copy( lsb/*XValue*/, result->m_actualExpressionValueStored.exprValue2BitVector.m_lsb);
                evaluationCompleted = true;
                convertResultToMemory=false;
            }
	    ...
	    if( operatorSymbol == "bitand" || operatorSymbol=="bitor" || operatorSymbol=="bitxor" || operatorSymbol=="bitxnor"||operatorSymbol=="bitnand")
	      {
		if(msb1.bv_width > msb2.bv_width)
		...
		else if(msb1.bv_width < msb2.bv_width)
		  {
            ...
            if(operatorSymbol == "replic")
            {
                int replic_times = -1;
                switch(operand2->m_typeInfo.m_type)
                {
                    case TYPE_UNSIGNED_INTEGER:
                        replic_times = operand2->getUnsignedIntValueOfExpression();
                        break;
                    case TYPE_SIGNED_INTEGER:
                        replic_times = operand2->getSignedIntValueOfExpression();
                        break;
                    case TYPE_UNSIGNED_BITVECTOR:
                    case TYPE_SIGNED_BITVECTOR:
                        replic_times = bvm->convertBVToUnsignedInt(msb2);
                        break;
                    default:
                        cerr<<operand2->m_typeInfo.m_type;
                        assert(false && "Unhandled type for replic operation");
                }
                assert(replic_times>=0);
                result->m_typeInfo.m_width = operand1->getWidth()*replic_times;
                if(replic_times>0)
                {
                    bvatom temp_msb=msb1,temp_lsb = lsb1;
                    replic_times--;
                    while(replic_times--)
                    {
                        temp_msb = bvm->concat(temp_msb,msb1);
                        temp_lsb = bvm->concat(temp_lsb,lsb1);
                    }
                    result->m_actualExpressionValueStored.exprValue2BitVector.m_msb = temp_msb;
                    result->m_actualExpressionValueStored.exprValue2BitVector.m_lsb = temp_lsb;
                    evaluationCompleted = true;
                }
                
            }
            else if (operatorSymbol == "bitand")
            {
            ...
            else if (operatorSymbol == "zeroextn")
            {
                ...
                int val = bvm->convertBVToUnsignedInt(msb2);
                val = val - msb1.bv_width;
                if (val <= 0)
                {
                    bvm->bv_copy(msb1,result->m_actualExpressionValueStored.exprValue2BitVector.m_msb);
                    bvm->bv_copy(lsb1,result->m_actualExpressionValueStored.exprValue2BitVector.m_lsb);
            ...
            else if (operatorSymbol == "signextn")
            {
                if (checkforZbit(msb2, lsb2) == true || checkforXbit(msb2, lsb2) == true)
                {
                    cerr << "signextn operator error,operand2 contains x or z value";
                    delete result;
                    exit(-1);
                }
                int val = bvm->convertBVToUnsignedInt(msb2);
                ...
                {
                    string zeroStr = "0",oneStr="1";
                    if(bvm->isMsb1(msb1) && !bvm->isMsb1(lsb1))
                    {
                        zeroStr.swap(oneStr);
                    }
                    bvatom zeroVector = getZerosOrOnes(val, zeroStr);
                    bvatom oneVector = getZerosOrOnes(val, oneStr);
                    bvatom res1 = bvm->concat(zeroVector, msb1);
                    result->m_actualExpressionValueStored.exprValue2BitVector.m_msb = res1;
                    bvatom res2 = bvm->concat(oneVector, lsb1);
                    result->m_actualExpressionValueStored.exprValue2BitVector.m_lsb = res2;
            ...
            else if (operatorSymbol == "logeq" || operatorSymbol == "diseq" ||
		...
		if(msb1.bv_width < msb2.bv_width)
		  {
                    cout<<"Extending operand1"<<endl;
		    msb1 = bvm->BVExtend(msb1, msb2.bv_width,0);
                    lsb1 = bvm->BVExtend(lsb1, lsb2.bv_width, 0);
                    
		  }
		else if(msb1.bv_width > msb2.bv_width )
		  {
                    cout<<"Extending operand2"<<endl;
		    msb2 = bvm->BVExtend(msb2, msb1.bv_width,0);
                    lsb2 = bvm->BVExtend(lsb2, lsb1.bv_width,0);
		  }
                 if (operatorSymbol == "logeq" && evaluationCompleted == false)
                {
                    /**
                    ...
                    if (msb1.bv_width != msb2.bv_width)
                    {
                        equal = bvm->checkBVEquality(bvm->reduction_and(msb1), bvm->reduction_and(msb2));
                    }
                    else
                    {
                        assert(msb1.bv_width == lsb1.bv_width);
                        assert(msb2.bv_width == lsb2.bv_width);
                        assert(msb1.bv_width == msb2.bv_width);
                 ...
                 else if (operatorSymbol == "diseq" && evaluationCompleted == false)
                {
                    /*
                 ...
                 else if(evaluationCompleted == false && (operatorSymbol=="gt" || operatorSymbol=="lt" || operatorSymbol=="ge" || operatorSymbol=="le" ) )
		  {
        ...
        else if (operands.size() == 3 && evaluationCompleted == false)
        {
                    ...
                    !operand2->isActualValueNULL() && !operand3->isActualValueNULL())
            {
                ...
                if (operand2->getOnlyTypeOfExpression() == TYPE_SIGNED_INTEGER &&
                        operand3->getOnlyTypeOfExpression() == TYPE_SIGNED_INTEGER)
                {
                ...
                if (flag == 1)
                {
                    if (msb1.bv_width <= op3Val)
                    {
                        bvatom temp_msb = bvm->BVExtend(msb1, op3Val + 1, 0);
                        result->m_actualExpressionValueStored.exprValue2BitVector.m_msb = bvm->select_with_int(temp_msb, op3Val, op2Val);
                        bvatom temp_lsb = getZerosOrOnes(op3Val + 1, "1");
                        bvm->replaceBySelBVatom(lsb1, temp_lsb, 0, 0, lsb1.bv_width);
                        assert(temp_msb.bv_width == temp_lsb.bv_width);
                        result->m_actualExpressionValueStored.exprValue2BitVector.m_lsb = bvm->select_with_int(temp_lsb, op3Val, op2Val);
                    ...
                    {
                        bvatom temp = bvm->select_with_int(msb1, op3Val, op2Val);
                        bvatom lsbVal = bvm->select_with_int(lsb1, op3Val, op2Val);
                        result->m_actualExpressionValueStored.exprValue2BitVector.m_msb = temp;
...
t_ExpressionValue * t_ExpressionEvaluator::performEvaluationOnGivenOperandsWithOneBit(string &operatorSymbol, vector<t_ExpressionValue *> &operands)
{
    ...
    else if (operands.size() == 3) // OPERATORS with 3 OPERANDS
    {
                ...
                !op2->isActualValueNULL() && !op3->isActualValueNULL())
        {
            ...
            result->m_typeInfo.m_width = abs(op2Val - op3Val)+1;
...
t_ExpressionValue * t_ExpressionEvaluator::performUpdateOperationOnMemory(vector<t_ExpressionValue*> &operandsOfUpdate)
{
    ...
    else
    {
        ...
        cout<<"Updating with "<<result->getActualMemoryAsString(listType, listType->m_bounds);
    ...
    result->m_typeInfo.m_type = TYPE_MEMORY_ARRAY;
    result->m_typeInfo.m_width = -1;
    //cout<<"Insert value is :"<<
    memList->updateAnEntryInListType(index, insertValue);
    result->m_actualExpressionValueStored.exprvalueMemory = (t_MemoryDataType*) memList;
    
    cout<<" at index:"<<index<<" result:"<<result->getActualValueAsString()<<endl;
    return result;
}
...
t_ExpressionValue * t_ExpressionEvaluator::performReadOperationOnMemory(vector<t_ExpressionValue*> &operandsOfUpdate)
{
    ...
    cout<<"performing read on index is :"<<index<<endl;
    ...
    cout<<"Actual value of read:"<< result->getActualValueAsString()<<endl;
    return result;
}
