...
////                    }
        ...
        if (type == "compound") {
        myfile << nid << " [shape = circle, penwidth=2, tooltip = \"" << display_names << "\"," << " label = \"" << display_names << "\\n(" + all_ids + ")" << "\"";
    } else if (type == "group") {
        //cout << "Do I xdot groups" << endl;
        myfile << nid << " [shape = rectangle, tooltip = \"" << display_names << "\"," << " label = \"" << display_names << "\\n(" + all_ids + ")" << "\"";
    } else if (type == "phenotype") {
        myfile << nid << " [shape = house, penwidth=2, peripheries=2, tooltip = \"" << display_names << "\"," << " label = \"" << display_names << "\"";
    } else if (type == "reaction") {
        std::string desc = get_description_for_node(nid);
        ...
        //myfile << nid << " [style = \"rounded, filled\", color = grey, margin = 0.05, shape = box, tooltip = \"" << display_names << "\"," << " label = \"" << display_names + "\\n(" + all_ids + ")" << "\"";
        myfile << nid << " [style = \"rounded, filled\", color = grey, margin = 0.05, shape = box, height = 1, width = 2, penwidth=5, tooltip = \"" << display_names << "\"," << " label = " <<
                "< <TABLE BORDER=\"0\" CELLBORDER=\"0\" CELLSPACING=\"0\"> " <<
                "<TR><TD>" << display_names << "</TD></TR>" << endl;
                for(int i = 0; i < all_id_vec.size(); i++){
                    //myfile << "<TR><TD TOOLTIP=\"" << display_names << "\" ";
                    string curr_id = all_id_vec.at(i);
                    //indiv_ids = split_string_into_list(curr_id, '.');
...
void GraphNew::write_node_to_grml_file(int nid, map<int, int>& nid_to_ml_node_id_map, std::ofstream &fout, string color_map_filename) {
...
void GraphNew::write_edge_to_grml_file(int eid, map<int, int>& eid_to_ml_edge_id_map, std::ofstream &fout, std::string nme) {
    ...
    */
    ...
    fout<<"<data key=\"URL\">file://"<<nme<<"#"<<eid<<"</data>"<<endl;
...
void GraphNew::write_to_xml_file(std::ofstream &fout, bool b_mode, string p_name) {
    string pathway;
    if(!b_mode){
...
GraphManagerNew::GraphManagerNew(string ip_file) {
    //while (true) {
...
//        }
        ...
        for(itr = NIDSg1.begin(); itr != NIDSg1.end(); itr++) {
            ...
            vector<string> dis_names = graph1->get_all_display_ids_of_node(*itr);
            vector<string> all_id = graph1->get_all_ids_of_node(*itr);
            for (vector<string>::iterator all_id_itr = all_id.begin(); all_id_itr != all_id.end(); all_id_itr++) {
                //string rep_id = graph1->get_rep_id_from_id(*all_id_itr);
                ...
                if (new_nid == -1) {
                    ...
                    if (node_type == "group") {
                        ...
                        new_graph->add_rep_id_for_node(new_nid, complex_id); // added by sukanya
                        new_graph->add_id_rep_id(complex_id, complex_id); // commented out by sukanya
                        new_graph->add_node_type(new_nid, node_type);
        ...
        for(itr = NIDSg2.begin(); itr != NIDSg2.end(); itr++) {
            ...
            vector<string> dis_names = graph2->get_all_display_ids_of_node(*itr);
            
            // create node
            vector<string> all_id = graph2->get_all_ids_of_node(*itr);
            for (vector<string>::iterator all_id_itr = all_id.begin(); all_id_itr != all_id.end(); all_id_itr++) {
                //string rep_id = graph2->get_rep_id_from_id(*all_id_itr);
                // --
                string temp_rep_id = *all_id_itr;
                string temp_rep_id2 = id_to_rep_id_graph3[*all_id_itr];
                while (temp_rep_id != temp_rep_id2) {
        ...
        for(itr = EIDSg1.begin(); itr != EIDSg1.end(); itr++) {
            ...
            int new_source_nid = new_graph->get_nid_from_rep_id(new_src_rep_id);
            int new_target_nid = new_graph->get_nid_from_rep_id(new_tgt_rep_id);
...
int GraphManagerNew::compute_gh_tree(int gid, vector<int>& gomoryhu_parents, vector<set<int> >& cut_edges){
...
set<int> GraphManagerNew::getMinCutEdges(GraphNew * graph, vector <set<int> >& cut_edges, vector<int>& gomoryhu_parents, int source_nid, int target_nid){
	int node_i = source_nid;
	int node_j = target_nid;
...
void GraphManagerNew::new_connect(GraphNew * graph, set<int>& present_edges, vector <set<int> >& cut_edges, vector<int>& gomoryhu_parents, int source_nid, int target_nid){
	set<int> curr_min_cut = getMinCutEdges(graph, cut_edges, gomoryhu_parents, source_nid, target_nid);
	set<int>::iterator set_itr;
	cout << "Current min cut contains edges:" << endl;
	...
	if (!graph->edge_has_subtype(chosen_edge_id, "dummy")) {
    	...
    	if (source_nid != chosen_edge->get_source())
        	new_connect(graph, present_edges, cut_edges, gomoryhu_parents, source_nid, chosen_edge->get_source());
    
    	if(target_nid != chosen_edge->get_target())
       		new_connect(graph, present_edges, cut_edges, gomoryhu_parents, chosen_edge->get_target(), target_nid);
...
int GraphManagerNew::read_graph_xml_file(std::string input_file, int thres, bool tool_written) {
    GraphNew* graph_ptr = new GraphNew;
    int gid = ++GraphManagerNew::graph_id_count;
    graph_ptr->set_graph_id(gid);
    add_graph(gid, graph_ptr);
    XMLParser *my_parser = new XMLParser(); //for parsing input xml file from KEGG
    if (my_parser->fill_graph_from_xml_file(graph_ptr, input_file, this, thres, tool_written)) {
        //add component edges from components to complexes
        if (!tool_written) {
                graph_ptr->add_component_edges_for_complexes(this);
        }
...
void GraphManagerNew::testing_printexpr_iter(t_ExpressionManager* em) {
 ...
 void GraphNew::convert_to_graphml( std::ofstream &fout, bool b_mode, vector<string> &token_cmd, int &cmd_index ) {
    ...
    for (vector<int>::iterator nid_itr = NIDSg.begin(); nid_itr != NIDSg.end(); ++nid_itr)    {
        ...
        {
                //cout<<*nid_itr;
                write_node_to_grml_file(*nid_itr, nid_to_ml_node_id_map, fout, color_map_filename);
        }
        count++;
    }
    ...
    hout<<"<html>"<<endl;
    hout<<"<head>"<<endl;
    hout<<"<title>This file contains all mincut information for a graph</title>"<<endl;
    hout<<"</head>"<<endl;
    hout<<"<body>"<<endl;
 ...
 void GraphNew::write_edge_to_html(int eid, map<int, int>& eid_to_ml_edge_id_map, std::ofstream &hout) 
 {
     ...
     for (unordered_map<int, EdgeNew *>::iterator edge_map_itr = eid_to_edge_map.begin(); edge_map_itr != eid_to_edge_map.end(); ++edge_map_itr) {
    vector<string> edge_pathways = edge_map_itr->second->get_pathways();
    print_vector(edge_pathways, hout);
    }
     hout<<"</p>"<<endl;
     hout<<"\n"<<endl;
