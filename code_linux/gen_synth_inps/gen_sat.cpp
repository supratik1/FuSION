...
public:
  c_GenSynthGraphs(int s, int n, int nrb, int erb, string wd, int to1, int to2) {
    unsat_option = s;
    nvars = n;
...
int main(int argc, char *argv[])
{
  if ((argc < 6) or (argc > 8)) {
    ...
    cerr << "Usage: " << string(argv[0]) << " unsat_option[0/1] num_vars node_relax_bound edge_relax_bound working_directory [z3_timeout1 z3_timeout2]" << endl;
  }
  else {
    c_GenSynthGraphs sG(stoi(argv[1]), stoi(argv[2]), stoi(argv[3]), stoi(argv[4]), string(argv[5]), (argc >= 7)?stoi(argv[6]):30000, (argc >= 8)?stoi(argv[7]):150000);
...
void c_GenSynthGraphs::genInputScriptFile()
{
  ...
  sf << "let $CONSTR_SOLVER_TIMEOUT1 " << to_string(timeout1) << endl;
  sf << "let $CONSTR_SOLVER_TIMEOUT2 " << to_string(timeout2) << endl << endl;
